...
export class Serv1Service {
	...
	async verifyUserByOrder(para: string, order: number): Promise<Result> {
		...
		var strs = ['user.userName = :userName',
		...
		switch (order) {
			case 0:
				let u0 = await con.where(strs[0], { userName: para })
					.printSql()
					.getOne();
				if ( u0 != null ) {
					console.dir(u0);
			...
			case 1:
				let u1 = await con.where(strs[1], { userEmail: para })
					.printSql()
					.getOne();
				if (u1 != null ) {
					console.dir(u1);
			...
			case 2:
				let u2 = await con.where(strs[2], { phoneNum: para })
					.printSql()
					.getOne();
				if (u2 != null ) {
					console.dir(u2);
